...
const MetadataEditor: React.FC<{ settings: ArticleSettings, onChange: (s: ArticleSettings) => void }> = ({ settings, onChange }) => {
  const safeSettings: ArticleSettings = settings || { headerEnabled: false, footerEnabled: false };
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between p-3 bg-stone-50 rounded border border-stone-200">
          <div className="flex items-center gap-2">
            <Star size={16} className={safeSettings.featured ? "text-amber-500 fill-amber-500" : "text-stone-400"} />
            <div>
              <label className="text-sm font-bold text-stone-900 cursor-pointer select-none">
                Recommandé
              </label>
              <p className="text-[10px] text-stone-500">Mettre en avant cet article</p>
            </div>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              checked={safeSettings.featured || false}
              onChange={(e) => onChange({ ...safeSettings, featured: e.target.checked })}
        ...
        {safeSettings.featured && (
          <div className="flex items-center gap-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded animate-fade-in">
            <Star size={14} className="text-amber-600 fill-amber-600" />
            <span className="text-xs font-bold text-amber-900">⭐ Recommandé</span>
          </div>
        )}

        {/* Publication Date */}
        <div>
          <label className="flex items-center gap-2 text-[10px] font-bold text-stone-500 uppercase mb-2">
            <Calendar size={12} />
            Date de publication
          </label>
          <input
            type="date"
            value={safeSettings.date || today}
            onChange={(e) => onChange({ ...safeSettings, date: e.target.value })}
            className="w-full p-2 border border-stone-200 rounded bg-white text-stone-900 font-sans text-sm focus:border-stone-800 focus:ring-1 focus:ring-stone-800 focus:outline-none transition-colors"
          />
        </div>
        ...
        <div>
          ...
          <select
            value={safeSettings.category || "Commerces & lieux"}
            onChange={(e) => onChange({ ...safeSettings, category: e.target.value as any })}
            className="w-full p-2 border border-stone-200 rounded bg-white text-stone-900 font-sans text-sm focus:border-stone-800 focus:ring-1 focus:ring-stone-800 focus:outline-none transition-colors cursor-pointer"
          >
            <option value="Beauté & cosmétique">Beauté & cosmétique</option>
            <option value="Commerces & lieux">Commerces & lieux</option>
            <option value="Événementiel">Événementiel</option>
            <option value="Échapées Belles">Échapées Belles</option>
          </select>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              type="number"
              min="1"
              value={safeSettings.readingMinutes ?? ''}
              onChange={(e) => {
                const val = e.target.value === '' ? null : parseInt(e.target.value, 10);
                onChange({ ...safeSettings, readingMinutes: val });
              }}
              placeholder="Auto"
              className="w-full p-2 pr-12 border border-stone-200 rounded bg-white text-stone-900 font-sans text-sm focus:border-stone-800 focus:ring-1 focus:ring-stone-800 focus:outline-none transition-colors placeholder:text-stone-400"
            />
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-stone-400 font-bold pointer-events-none">min</span>
          </div>
          <p className="text-[10px] text-stone-400 mt-1.5 italic">
            Optionnel — si vide, il sera calculé automatiquement.
          </p>
        </div>
      </div>
    </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {layouts.map((l) => (
          <button
            ...
            className={`flex flex-col items-center gap-1 p-2 rounded border transition-all ${(current === l.id || (!current && l.id === 'two_thirds'))
...
const BlockEditor: React.FC<BlockEditorProps> = ({
  ...
  // Sync helpers
  const mapLayoutToPlacement = (l: TextLayout): Placement => {
    if (l === 'left_third') return 'left';
    if (l === 'right_third') return 'right';
    return 'full';
  };
  const mapPlacementToLayout = (p: Placement): TextLayout => {
    if (p === 'left') return 'left_third';
    if (p === 'right') return 'right_third';
    return 'full';
  };

  const mapPositionToPlacement = (pos: 'left' | 'center' | 'right'): Placement => {
    if (pos === 'left') return 'left';
    if (pos === 'right') return 'right';
    return 'full';
  };
  const mapPlacementToPosition = (p: Placement): 'left' | 'center' | 'right' => {
          ...
          <div className="space-y-3" onPointerDownCapture={stopProp}>
            <div>
              ...
              <input
                ...
                className="w-full p-2 border-b border-stone-200 font-serif text-2xl bg-transparent focus:border-stone-800 focus:outline-none transition-colors"
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-2 border-b border-stone-200 font-sans text-xs uppercase tracking-widest bg-transparent focus:border-stone-800 focus:outline-none transition-colors"
          ...
          <div className="space-y-3" onPointerDownCapture={stopProp}>
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-2 border-b border-stone-200 font-serif text-lg bg-transparent focus:border-stone-800 focus:outline-none"
...
export const AdminBuilder: React.FC<AdminBuilderProps> = ({
  ...
  return (
    <div className="max-w-[800px] mx-auto pb-32 pt-10">
      ...
      <MetadataEditor settings={settings} onChange={setSettings} />
